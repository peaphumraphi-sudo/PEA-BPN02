...
import { Layout, Menu, LogIn, Package, Truck, ClipboardCheck, Bell, LogOut, ChevronRight, QrCode, Plus, Minus, Download, User as UserIcon, Activity } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'main' | 'vehicle' | 'checklist' | 'users' | 'help'>('dashboard');
  ...
  useEffect(() => {
    ...
    if (savedGoogleTokens) setGoogleTokens(JSON.parse(savedGoogleTokens));
  ...
  useEffect(() => {
    ...
    if (googleTokens) localStorage.setItem('google_tokens', JSON.stringify(googleTokens));
  }, [inventory, vehicles, users, googleTokens]);
  ...
  const handleConnectGoogle = async () => {
    try {
      ...
      alert('ไม่สามารถเชื่อมต่อ Google ได้ กรุณาตรวจสอบการตั้งค่า Environment Variables');
    }
  };
  ...
  const handleSyncToSheets = async () => {
    ...
    const spreadsheetId = prompt('กรุณาใส่ Google Spreadsheet ID (ดูได้จาก URL ของไฟล์)');
    if (!spreadsheetId) return;

    // Prepare data for sync
    const values = [
      ['รหัสพัสดุ', 'ชื่อพัสดุ', 'หมวดหมู่', 'จำนวนคงเหลือ', 'หน่วย', 'จุดเตือนขั้นต่ำ'],
      ...inventory.map(item => [item.id, item.name, item.category, item.quantity, item.unit, item.minThreshold])
    ];
    ...
    try {
      ...
      alert('ซิงค์ข้อมูลล้มเหลว: ' + error.message);
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 pb-20 md:pb-0">
          ...
          <ul className="space-y-1 px-3">
            ...
            <li>
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'help' ? 'bg-purple-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}
              >
      ...
      <nav className="md:hidden fixed bottom-4 left-4 right-4 bg-white/90 backdrop-blur-lg border border-white/20 z-50 px-2 py-2 flex justify-around items-center shadow-2xl rounded-3xl pb-safe">
        ...
        <button 
          onClick={() => setActiveTab('help')}
          className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all duration-300 ${activeTab === 'help' ? 'text-purple-600 bg-purple-50' : 'text-slate-400'}`}
        >
          <Activity className={`w-6 h-6 ${activeTab === 'help' ? 'scale-110' : ''}`} />
          <span className="text-[10px] font-bold">ช่วยเหลือ</span>
        </button>
      </nav>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl md:text-3xl font-black text-slate-800 tracking-tight">
                  ...
                  {activeTab === 'help' && 'ศูนย์ความช่วยเหลือ'}
                </h2>
            ...
            <div className="space-y-6 pb-20">
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <h3 className="text-xl font-black text-slate-800 mb-4 flex items-center gap-2">
                  ...
                  วิธีใช้งานระบบ PEA BPN
                </h3>
                <div className="space-y-4">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="font-bold text-purple-700 mb-1">1. การเพิ่มพัสดุ</p>
                    <p className="text-sm text-slate-600">ไปที่เมนู "คลังหลัก" เพื่อเพิ่มรายการพัสดุจริง รหัสพัสดุ (ID) ที่คุณตั้งไว้จะถูกใช้เป็นข้อมูลใน QR Code</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="font-bold text-purple-700 mb-1">2. การเบิกของเข้ารถ</p>
                    <p className="text-sm text-slate-600">ไปที่เมนู "พัสดุประจำรถ" เลือก "เบิกของเข้ารถ" แล้วแสกน QR Code ที่ชั้นวางพัสดุ</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="font-bold text-purple-700 mb-1">3. การตรวจนับประจำวัน</p>
                    <p className="text-sm text-slate-600">ใช้เพื่อยืนยันยอดพัสดุที่เหลืออยู่จริงบนรถหลังจบงาน ระบบจะบันทึกประวัติไว้ตรวจสอบได้</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="font-bold text-purple-700 mb-1">4. การซิงค์ Google Sheets</p>
                    <p className="text-sm text-slate-600">เชื่อมต่อบัญชี Google ในหน้า Dashboard และใส่ Spreadsheet ID เพื่อส่งข้อมูลไปเก็บไว้บนคลาวด์</p>
                  </div>
                </div>
              </div>
              
              <div className="bg-purple-50 rounded-3xl p-6 border border-purple-100">
                <h3 className="font-black text-purple-900 mb-2">คำแนะนำเพิ่มเติม</h3>
                <ul className="text-sm text-purple-800 space-y-2 list-disc pl-4">
                  <li>ควรพิมพ์ QR Code ติดไว้ที่ชั้นวางพัสดุเพื่อให้แสกนได้ง่าย</li>
                  <li>สำรองข้อมูล (Backup) สม่ำเสมอผ่านหน้า Dashboard</li>
                  <li>หากต้องการใช้งานเป็นแอป ให้กด "Add to Home Screen" ในเบราว์เซอร์มือถือ</li>
                </ul>
              </div>
            </div>
          )}
